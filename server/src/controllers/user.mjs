...
const getFriends = async (req, res) => {
  try {
    ...
    const user = await User.findById(id).populate('friends', 'name username email avatar');

    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }

    res.status(200).json({ message: 'Friends fetched successfully', items: user.friends });
  } catch (err) {
    console.log(err);
    res.status(500).json({ message: "Internal server error!" });
  }
